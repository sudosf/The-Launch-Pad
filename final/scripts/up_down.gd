...
onready var option_scene = preload("res://final/components/up_down_option.tscn")
...
func populate_options(data):
	...
	for option_data in data:
		...
		if count % 2 == 0:
			...
			option_instance.get_node("offset").text = str(pos_offset - 200)
		
		pos_offset += 200
		count += 1
		$HBoxContainer.add_child(option_instance)
